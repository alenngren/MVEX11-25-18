...
densitet_evo013 <-density(X_evo03, sigma = 0.6) 
kordinater <- as.data.frame(densitet)
kordinater_evo013 <- as.data.frame(densitet_evo013)
colnames(kordinater) <- c("x", "y", "ca")

kordinater$ca <- pmax(kordinater_evo013$ca, 0)
lgcp <- gam(ca ~ s(x, y, k = 50), family = poisson, data = kordinater)
lgcp_evo <- gam(ca ~ s(x_evo03, y_evo03, k = 50), family = poisson, data = kordinater)
...
LGCP_evo013 <- rpoispp(lambda = skalad_lamda2, win = window) # X patter

# this is the ppp pattern. Plot L med mats data 
plot(LGCP)

E_lgcp <- envelope(LGCP, Kest, nsim=999, verbose=TRUE)
E_lgcp_evo013 <- envelope(LGCP_evo013, Kest, nsim=999, verbose=TRUE)
E_evo013 <- envelope(X_evo03, Kest, nsim=999, verbose=TRUE)
...
lines(E_lgcp$r, sqrt(E_lgcp$obs/pi) -E_lgcp$r , col = jet_colors[1], lty = 2, lwd = 2)
lines(E_lgcp_evo013$r, sqrt(E_lgcp_evo013$obs/pi) -E_lgcp_evo013$r , col = jet_colors[1], lty = 2, lwd = 2)
lines(E_evo013$r, sqrt(E_evo013$obs/pi) -E_evo013$r , col = jet_colors[2], lty = 2, lwd = 2)
...
X <- rLGCP("gauss", m, var = 0.6, scale = 10, win = win, saveLambda = TRUE) # subject to change 
...
# Interest of change, n_f, t,A och barn_std for summary statistics 
n_f <- 160
f_punkt <- data.frame(
...
plot(f_punkt$x, f_punkt$y, asp=1)


X_latent <- ppp(barn$x, barn$y, window=square(40))
E_lgcp_latent <- envelope(X_latent, Kest, nsim=99)



plot(E, sqrt(./pi) -r  ~ r, main = 'L summary ~ lambda och iterativ ', legend = FALSE) 
lines(E_lgcp_latent$r, sqrt(E_lgcp_latent$obs/pi) - E_lgcp_latent$r, col = jet_colors[2], lty = 2, lwd = 2)
lines(E_10$r, sqrt(E_10$obs/pi) -E_10$r , col = jet_colors[1], lty = 2, lwd = 2) 

legend("topleft", legend = c("data", "lambda field", "Iterativ"), 
       col = c(1, jet_colors[2], jet_colors[1]), lty = c(1, 2, 2))

# E_lgcp_latent <- envelope(X_latent, Lest, nsim=99)
plot(E_lgcp_latent)
